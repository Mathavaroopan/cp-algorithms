...
void solve(){
    ...
    int ans = 0;
    for(int i = 0; i < n; i++){
        // cout << "I'm " << arr[i] << " Target : " << target << endl;
        // cout << "I want : " << abs(arr[i] - target) << endl;
        int find = 0;
        if(abs(arr[i] - target) == (int)abs(arr[i] - target)){
            find = (int)abs(arr[i] - target);
        }else{
            find = abs(arr[i] - target);
        }
        if(find == 0){
            if(mp.count(abs(arr[i] - target))){
                if(abs(arr[i] - target) == arr[i]){
                    if(mp[abs(arr[i] - target)] > 1) ans++;
                }else{
                    ans++;
                }
            }
