...
bool check(int mid, vector<int>& d, vector<int>& p){
    vector<int> studied(k, -1);
    for(int i = 0; i < mid; i++) if(d[i]) studied[d[i] - 1] = i;
    for(int i : studied) if(i == -1) return false;
    int sum = 0;
    for(int i = 0; i < mid; i++){
        if(d[i] && studied[d[i] - 1] == i) sum -= p[d[i] - 1];
        else sum++;
