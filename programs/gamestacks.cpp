...
int solve(int i, int j, int maxSum, int curSum, vector<int>& a, vector<int>& b, vector<vector<int>>& dp){
    if(i == a.size()){
        int temp = 0;
        while(j < b.size() && curSum + b[j] <= maxSum){
            curSum += b[j];
            temp++;
            j++;
        }
    ...
    if(j == b.size()){
        int temp = 0;
        while(i < a.size() && curSum + a[i] <= maxSum){
            curSum += a[i];
            temp++;
            i++;
        }
    ...
    if(curSum + a[i] <= maxSum){
        left = 1 + solve(i + 1, j, maxSum, curSum + a[i], a, b, dp);
    }
    if(curSum + b[j] <= maxSum){
        right = 1 + solve(i, j + 1, maxSum, curSum + b[j], a, b, dp);
    }
...
int32_t main()
{
 ...
 cin.tie(NULL);
    ...
    while (T--)
    {
        ...
        cout << solve(0, 0, maxSum, 0, a, b, dp);
    }
    return 0;
}
