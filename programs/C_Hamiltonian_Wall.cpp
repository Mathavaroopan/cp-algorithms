...
vector<int> dp[2];
vector<int> vis[2];
bool rec(int row, int col, int cur){
    if(cur == sum) return true;
    cout << row << " " << col << endl;
    vis[row][col] = 1;
    bool ans = false;
    if(row + 1 < 2){
        if(arr[row + 1][col] == 1 && vis[row + 1][col] == 0) ans = ans || rec(row + 1, col, cur + 1);
    }
    if(row - 1 >= 0){
        if(arr[row - 1][col] == 1 && vis[row - 1][col] == 0) ans = ans || rec(row - 1, col, cur + 1);
    }
    if(col + 1 < n){
        if(arr[row][col + 1] == 1 && vis[row][col + 1] == 0) ans = ans || rec(row, col + 1, cur + 1);
    }
    vis[row][col] = 0;
    return ans;
}
